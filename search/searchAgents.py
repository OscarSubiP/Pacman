...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        #init var
        self.cornerVisitedList = [False, False, False, False]
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.cornerVisitedList)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not isWall:
                ...
                for corner in self.corners: #get the corner num
                    if (new_x, new_y) == corner:
                        break
                    numCorner += 1

                if numCorner < 4:   #if is a corner, change value to true
                    cornersVisited[numCorner] = True

                newState = ((new_x, new_y), cornersVisited)
    ...
    while cornersUnexplored != []:
        ...
        for candidateCorner in cornersUnexplored:
            ...
            if manhattan < minManhattan:    #get the nearest corner and the min manhattan
                ...
                nearCorner = candidateCorner
        
        distancePath += minManhattan    #add the min manhattan to de distance path
        cornersUnexplored.remove(nearCorner)    #remove the visited corner of the unexplored list
        currentState = nearCorner   #change the corner to explore
...
def foodHeuristic(state, problem):
    ...
    for posFood in foodGrid.asList():
        distance = util.manhattanDistance(position, posFood)    #get the distance with the manhattan
        distances.append(distance)  #add the distance to de list

    distancePath = max(distances)   #get the max distance
